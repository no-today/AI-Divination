...
const App: React.FC = () => {
  ...
  const performDivination = useCallback(async () => {
    ...
    try {
      // Ritualistic delay - Extended to 0.5s for the slower, breathable cycle
  ...
  return (
    // Root Container: Native Document Scroll
    // Switched from 'fixed overflow-y-auto' to 'min-h-screen relative' to allow native browser scrolling.
    // This fixes mobile touch event blocking on inner elements.
    <div className="min-h-screen w-full bg-[#F7F5F0] selection:bg-[#A63437] selection:text-white overflow-x-hidden relative">
      
      {/* Background Atmosphere - Daoist Void */}
      {/* Fixed position relative to viewport, stays static while content scrolls */}
      ...
      {/* Content Wrapper */}
      {/* min-h-screen ensures it fills the viewport height to allow centering in IDLE state */}
      {/* justify-start in RESULT mode ensures content flows naturally from the top */}
      <div className={`relative z-10 w-full min-h-screen flex flex-col items-center transition-all duration-500 ${state === AppState.RESULT ? 'justify-start py-12' : 'justify-center'}`}>
        ...
        <main className="w-full max-w-2xl px-6 flex flex-col items-center">
          ...
          {state === AppState.IDLE && (
            <div className="w-full flex flex-col items-center animate-[fadeIn_2s_ease-out] py-10">
              ...
              {/* Input Section - Minimalist Style */}
              <div className="relative w-full max-w-sm mt-8">
                <input
                  type="text"
                  className="w-full bg-transparent text-center text-xl font-serif tracking-widest text-[#1A1A1A] placeholder-[#A09B93] border-b border-[#E5E5E5] focus:border-[#BC3632] focus:outline-none py-3 transition-colors duration-300 caret-[#BC3632]"
                  placeholder="所问何事"
                  value={query}
